...
from src.Journals.views.tasks import process_uploaded_file_task

class PaperProcessorAPI(Resource):
    def post(self):
        """
        Accepts a CSV file from the user, validates it, and triggers a background task for processing.
        """
        ...
        task = process_uploaded_file_task(temp_file_path, output_file_path)
        print(task)
        # task = process_uploaded_file_task.apply_async(args=[temp_file_path, output_file_path])

...
class UniversalExtractor(BaseArticleExtractor):
    def _extract_sections(self):
        """Extract sections for articles across different sources."""
        # Extract title and ensure it's added to sections_dict
        self._extract_title()
        self.is_open_access()
    ...
    def _extract_image_content(self):
        ...
        if extracted_content and extracted_content != "":
            ...
            for svg in self.soup.find_all("svg"):
                # Extract text from <text> elements inside <svg>
                texts = [text.get_text(separator=" ", strip=True) for text in svg.find_all("text")]
                if texts:
                    svg_texts.append("\n".join(texts))

            # Combine extracted image and SVG text content
            self.image_texts = extracted_content + "\n\n".join(svg_texts) if svg_texts else extracted_content
    ...
    def _extract_title(self):
        ...
        if self.soup is not None:
            title_tag = self.soup.find('h1', {'class': 'title'}) or self.soup.find('title') or self.soup.find('h1', {'property': 'name'})
            if title_tag:
                self.sections_dict["title"] = f"===== Title =====\n" + title_tag.get_text(strip=True)
            elif not title_tag or title_tag != "":
                title_tag = self.soup.find('meta', {'name': 'citation_title'}) or self.soup.find('title')
                if title_tag:
                    self.sections_dict["title"] = f"===== Title =====\n" + title_tag.get("content", "").strip()
        else:
            print("Failed to initialize BeautifulSoup object")
    ...
    def __extract_sections(self):
        ...
        section_aliases = {
            ...
            "search_strategy": [
                ...
                r"literature*search*strategy\s*and\s*study*selection",
                r"data\s*sources?",
                r"search\s*terms?",
                r"database\s*search",
    ...
    def _combine_main_content(self):
        """Combine key sections into a single 'MainContent' section."""
